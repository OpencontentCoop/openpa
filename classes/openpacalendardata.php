...
class OpenPACalendarData
{
    ...
    protected $typeEventIdentifier = "argomento"; //argomento
    ...
    function __construct( eZContentObjectTreeNode $calendar )
    {
        ...
        self::$CUSTOM_FILTERS = array(
            'MANIFESTAZIONE' => OpenPASolr::generateSolrSubField( self::getSingleEventIdentifier(), 'name', 'string') . ':"Manifestazione"',
            'SPECIAL' => OpenPASolr::generateSolrField('special', 'boolean') . ':true'
        );
    }

    public static function getSingleEventIdentifier()
    {
        $identifier = OpenPAINI::variable( 'CalendarSettings', 'SingleEventAttributeIdentifier', "tipo_evento" ); // per Trento è tipo_eventi_manifestazioni
        return $identifier;
    }

    public static function getEventTypeIdentifier()
    {
        $identifier = OpenPAINI::variable( 'CalendarSettings', 'EventTypeAttributeIdentifier', "argomento" ); // per Trento è tipo_evento
        return $identifier;
    }
    ...
    public function fetch()
    {
        ...
        $this->parameters['search_from_solr'] = $startDateTime->format('Y-m-d\TH:i:s\Z');
        $this->parameters['search_from_timestamp'] = $startDateTime->getTimestamp();

        // end day
        $endDateTime = clone $startDateTime;
        $endDateTime->add( $interval );
        $this->parameters['search_to_solr'] = $endDateTime->format('Y-m-d\TH:i:s\Z');
        ...
        $this->filters[] = '-' . OpenPASolr::generateSolrSubField(self::getSingleEventIdentifier(), 'name', 'string').':"Manifestazione"';
    ...
    {
        $default = array(
            self::getEventTypeIdentifier() => 'Tipologia',
            'materia' => 'Materia',
            'io_sono' => 'Destinatari',
