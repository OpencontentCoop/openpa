<h4 class="attributetype">{"Receiver"|i18n( 'survey' )}</h4>
...
<div class="form-group">
   ...
   <p class="help-block">{"N. B. If you enter just one email address, user will not see this question. Instead the posting
...
<table class="table table-condensed table-striped" cellspacing="0">
  ...
  <tr class="{$class}">
	...
	<td><input class="form-control" name="{$prefix_attribute}_ezsurvey_receiver_{$question.id}_{$option.id}_label_{$attribute_id}" type="text" value="{$option.label|wash('xhtml')}" size="30" /></td>
	<td><input class="form-control" name="{$prefix_attribute}_ezsurvey_receiver_{$question.id}_{$option.id}_value_{$attribute_id}" type="text" value="{$option.value|wash('xhtml')}" size="20" /></td>
	...
	<td><input class="form-control" name="{$prefix_attribute}_ezsurvey_receiver_{$question.id}_{$option.id}_tab_order_{$attribute_id}" type="text" size="2" value="{$option.id|wash('xhtml')}" /></td>
  </tr>
  {/foreach}
</table>
