	...
	<div class="blocco-titolo-oggetto">    
 		<div class="titolo-blocco-titolo">
			{if $node.class_identifier|eq('link')}
        			<h3>
                        <a href="{$node.data_map.location.content}" target="_blank" title="Apri il link '{$node.name|wash()}' in una pagina esterna (si lascerà il sito)">{$node.name|wash()}</a>
                        {include name=edit node=$node uri='design:parts/openpa/edit_buttons.tpl'} 
                    </h3>
			{else}
				{if is_set( $node.url_alias )}
                    <h3><a href={if $node.class_identifier|eq('area_tematica')}{$node.object.main_node.url_alias|ezurl}{else}{$node.url_alias|ezurl()}{/if} title="{$node.name|wash()}">{$node.name|wash()}</a></h3>
				{else}
        ...
        {elseif and(is_set($node.data_map.oggetto),$node.data_map.oggetto.has_content, $node.data_map.oggetto.content|ne($node.name))}
