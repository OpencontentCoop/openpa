...
*/
(function($){    
	$.fn.loadMenu = function(node_id, current_node_id, current_menu, ui_context, is_area_tematica, indexUri ) {
        // visualizza il menu con l'accordion via userParameters     
        this.load(indexUri+'/ezjscore/call/openpaajax::menu/(node_id)/'+node_id+'/(current_node_id)/'+current_node_id+'/(current_menu)/'+current_menu+'/(ui_context)/'+ui_context+'/(is_area_tematica)/'+is_area_tematica, function(){accordionMenu(this);})
        accordionMenu(this);
	}
