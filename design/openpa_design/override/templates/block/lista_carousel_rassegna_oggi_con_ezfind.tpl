...
{*
		...
		{def $custom_filter= array( array(concat('published:[', $curr_year, '-', $curr_month, '-13T00:00:00.000Z TO ', $curr_year, '-', $curr_month, '-', $curr_today,'T23:59:59.999Z]') ))}
		

	///JEK/// oggi = {$curr_today}/{$curr_month}/{$curr_year} --> custom_filter: {$custom_filter}
		...
		{def $children=fetch( ezfind,search,
                        hash( 'query', '',
                              'subtree_array', array(2),
			      'facet', hash('field', 'articolo/data_iniziopubblicazione',
					    'sort', 'alpha',
                              		    'limit', 60),
                              'class_id', array('articolo') ))}
		{*def $children=fetch( ezfind,search,
                        hash( 'query', '',
                              'subtree_array', array(2),
			      'facet', hash('field', 'articolo/data_iniziopubblicazione',
...
---> Count =  {$children|count()} ///
		...
		{*def $children=fetch( 'content', 'list', hash(
								'parent_node_id',$customs.node_id,
								'attribute_filter', array( array( 'articolo/data_iniziopubblicazione', '>', '$oggi' )),
