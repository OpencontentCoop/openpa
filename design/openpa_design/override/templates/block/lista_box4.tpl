{def $valid_nodes = $block.valid_nodes
	$valid_nodes_count = $valid_nodes|count()
        $curr_ts = currentdate()
	$children=array()
	$classes= ezini( 'GestioneClassi', 'classi_da_escludere_dai_blocchi_ezflow', 'content.ini')
}
	...
	<div class="border-content">
		...
		<h2>{$valid_nodes[0].name|wash()}</h2>
	        ...
	        {set $children=fetch( 'content', 'tree', hash( 'parent_node_id', $valid_nodes[0].node_id,
								...
								'depth',10, 'limit', 4, 'sort_by', array('published', false()) 
							      ) )}
		...
		{if $children|count()}
			<ul>							 
				...
				<li>
					<a href={$child.url_alias|ezurl()} title="{$child.data_map.abstract.content.output.output_text|explode("<br />")|implode(" ")|strip_tags()|trim()}">{$child.name}</a><div class="attribute-date">{$child.object.published|datetime(custom, '%j %F %Y')}</div>
				</li>
				{/foreach}
			</ul>
		{else}
