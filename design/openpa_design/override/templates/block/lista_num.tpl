{*
	Vista a tab con "numerini" in basso
				...
				{*if $classi_blocco_particolari|contains($child.class_identifier)*}
					...
					{if $child.class_identifier|eq('politico')}
						...
						{else}
							...
							{/if}
						{/if}
						{*attribute_view_gui attribute=$child.data_map.ricevimento*}
					...
					{elseif is_set($child.data_map.testata)}
					   ...
					   {if $child.data_map.testata.has_content}
					    ...
					    {if $child.data_map.argomento_articolo.has_content}
			 			<p>Su: 
						 <strong>
						 {attribute_view_gui is_area_tematica=$is_area_tematica href=nolink attribute=$child.data_map.argomento_articolo}
						 </strong>
						</p>
					    {/if}
					    </div>
