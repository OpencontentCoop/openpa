{*
{def $disf= ezini( 'GestioneAttributi', 'AttributiNonEditabili', 'content.ini')}

{if $disf|contains(concat($attribute.object.class_identifier,'/',$attribute.contentclass_attribute_identifier))} 
...
{let class_content=$attribute.class_content
     ...
     ufficio_utente = fetch( 'content', 'related_objects',  hash( 'object_id', $current_user.contentobject_id, 'attribute_identifier', 911,'all_relations', false() ))
...
*}
		...
		{* controlla che sia un servizio *}
                {if and($servizio_utente[0].id|gt(0), $attribute.contentclass_attribute_identifier|eq('servizio'))}
		...
		{* UFFICIO *}
			...
			{ezscript_require( array('ezjsc::jquery', 'jquery-ui/jquery.ui.core.min.js', 'jquery-ui/jquery.ui.widget.min.js', 'jquery-ui/jquery.ui.tabs.min.js') )}
			{literal}
			<script>
