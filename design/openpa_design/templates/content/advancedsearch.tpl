...
{def $latitude 				    = cond( ezhttp_hasvariable( 'latitude','get','hasVariable' ), ezhttp( 'latitude', 'get' ) )
	 ...
	 $OriginalNodeID 	    	= cond( ezhttp_hasvariable( 'OriginalNode','get','hasVariable' ), ezhttp( 'OriginalNode', 'get' ), 2 )
}

{* altre variabili di default *}
{def $ini_not_available_facets 	= openpaini( 'Classi', 'SearchEscludiDaFaccette', array() )
...
{if $date|gt( 0 )}
	{def $to_date = $date|sum(1)
         $f = setFilterParameter( 'published', concat('[', $date, '-01-01T00:59:59.999Z/YEAR TO ', $to_date, '-01-01T00:59:59.999Z/YEAR]') )}
{/if}
...
<div class="border-box search-page{if $OriginalNodeID|not()} search-full{/if} extrainfo">
...
<div class="border-ml"><div class="border-mr"><div class="border-mc float-break">
	{include name       = searchbox 
		...
		search_included = 1
		uri             = 'design:parts/openpa/search_class_and_attributes_block.tpl' }    
</div></div></div>
...
{if $ClassFilter|count()|eq(1)}
	...
	<div class="attribute-header">
        ...
        {else}
            in tutto il sito{/if} solo tra informazioni di tipo "{$contentClass.name}"</h1>
    </div>
